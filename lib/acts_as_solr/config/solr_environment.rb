...
SOLR_DATA_PATH = "#{rails_root_dir}/solr/#{ENV['Rails.env']}" unless defined? SOLR_DATA_PATH

unless defined? SOLR_PORT
  config = YAML::load_file(rails_root_dir+'/config/solr.yml')
  
  SOLR_PORT = ENV['PORT'] || URI.parse(config[ENV['Rails.env']]['url']).port
end

SOLR_JVM_OPTIONS = config[ENV['Rails.env']]['jvm_options'] unless defined? SOLR_JVM_OPTIONS

if ENV['Rails.env'] == 'test'
