...
namespace :solr do
  ...
  task :start do
    ...
    begin
      ...
      Dir.chdir(SOLR_PATH) do
        ...
        sleep(5)
        File.open("#{SOLR_PIDS_PATH}/#{ENV['Rails.env']}_pid", "w"){ |f| f << pid}
  ...
  task :stop do
    require "#{File.dirname(__FILE__)}/../../config/solr_environment.rb"
    fork do
      file_path = "#{SOLR_PIDS_PATH}/#{ENV['Rails.env']}_pid"
      if File.exists?(file_path)
        ...
        Rake::Task["solr:destroy_index"].invoke if ENV['Rails.env'] == 'test'
  ...
  task :destroy_index do
    require "#{File.dirname(__FILE__)}/../../config/solr_environment.rb"
    raise "In production mode.  I'm not going to delete the index, sorry." if ENV['Rails.env'] == "production"
    if File.exists?("#{SOLR_DATA_PATH}")
      Dir["#{SOLR_DATA_PATH}/index/*"].each{|f| File.unlink(f)}
      Dir.rmdir("#{SOLR_DATA_PATH}/index")
      puts "Index files removed under " + ENV['Rails.env'] + " environment"
    end
  end
