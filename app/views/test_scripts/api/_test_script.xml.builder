...
parent_xml.tag! "testScript" do
  ...
  # <submitter>
  parent_xml.submitter xlink_attributes(uri_for_object(test_script.user), :title => xlink_title(test_script.user)), 
                       :resourceType => "User",
                       :resourceName => display_name(test_script.user, false)
