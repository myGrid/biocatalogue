...
EOF
  ...
  def make_request(request)
    intialise_proxy_data
    result = nil
    
    http_session = @proxy.new(HOST, 443)
    http_session.use_ssl = true
    http_session.ca_file = SSL_CERT
  ...
  def intialise_proxy_data
    if @proxy_host.blank? or @proxy_port.blank? or @proxy.nil?
      proxy_uri = URI.parse("") 
      case HTTP_PROXY
        when URI
